# Diet and age effects on blood glucose

This directory contains the results of the analysis of diet and age effects on blood glucose. The direct effects of diet and age on glucose were estimated as well as the interaction between diet and age (i.e. the diet-specific effects of age). The effects were estimated both independently of bodyweight and conditioned on bodyweight.

The "figures" folder contains a large figure summarizing the diet and age effects for both glucose independent of weight and glucose conditioned on weight. Additionally, there are separate plots of glucose independent of weight and glucose conditioned on weight. The figures in the two plots are identical (they show the distribution of glucose across time broken down by diet group), but the captions give more detailed information about the test results which may differ between the tests that were conditioned on weight and those that were not. See the "Figures" section below for more details.
...
The results of the analysis of diet and age effects on blood glucose concentration are similar when conditioning on bodyweight and when ignoring bodyweight.

Blood glucose decreases with age for all groups, but the overall decrease from 5 to 28 months for the ad libitum and fasting mice is small. The decrease with age is signficantly greater for the caloric restricted (20% and 40%) compared to the ad libitum control mice and the rate of decrease in the 40% CR mice is significantly greater than all the other groups. 

However, the effect of diet on the change with age is primarily due to immediate responses to the diet intervention (started at 6 month) that manifest as differences in the change in blood glucose from 5 to 16 months. When looking only at the change from 16 to 28 months, the only groups have signficant differences in their rate of change with age are the caloric restriction groups compared to the 2-day fasting group. The caloric restriction groups show the greatest (though it is still relatively small compared to the change from 5 to 16 months) decrease with age, while mean blood glucose actually increases from 16 to 28 months in the 2-day fasting mice. 
...
The effects of diet and age on blood glucose were estimated using linear mixed models (R/lme4). The significance of age and diet effects were assessed with approximate F-tests using the Kenward and Roger (1997) approach (R/pbkrtest). The glucose values were not strongly skewed and were not transformed prior to model fitting and testing.
...
The direct effect of diet on glucose was estimated independently for each timepoint. When estimating these diet effects, collection date (test batch) was included as a random effect and diet was treated as a fixed effect. The significance of the diet effect independent of bodyweight was assessed by comparing the full and null models shown below:  
 - Full model: Glucose ~ (1 | DateCollect) + Diet
 - Null model: Glucose ~ (1 | DateCollect)   
...
The significance of the diet effect conditioned on bodyweight was assessed by comparing the full and null models shown below:  
 - Full model: Glucose ~ (1 | DateCollect) + Bodyweight + Diet
 - Null model: Glucose ~ (1 | DateCollect) + Bodyweight
 
If the results showed that glucose (indpendent of weight or conditioned on weight) is significantly (p < 0.05) affected by diet at a timepoint, the significance of the pairwise differences between each diet at that timepoint were assessed using a Tukey correction for multiple tests (R/emmeans).


<br>

 The direct effect of age on glucose was estimated using a linear mixed model with diet, collection date (batch), and mouse ID as random effects and age in months as a fixed effect. The significance of the age effect independent of bodyweight was assessed by comparing the full and null models shown below:
 - Full model: Glucose ~ (1 | MouseID) + (1 | DateCollect) + (1 | Diet) + Age
 - Null model: Glucose ~ (1 | MouseID) + (1 | DateCollect) + (1 | Diet)  

The significance of the age effect conditioned on bodyweight was assessed by comparing the full and null models shown below:  
 - Full model: Glucose ~ (1 | MouseID) + (1 | DateCollect) + (1 | Diet) + Bodyweight + Age
 - Null model: Glucose ~ (1 | MouseID) + (1 | DateCollect) + (1 | Diet) + Bodyweight  


 <br>

 The interaction effect of age and diet on glucose was estimated using a linear mixed model with collection date (batch) and mouse ID as random effects and diet, age in months, and the interaction between diet and age in months as fixed effects. The significance of the age-diet interaction effect independent of bodyweight was assessed by comparing the full and null models shown below:
 - Full model: Glucose ~ (1 | MouseID) + (1 | DateCollect) + Diet + Age + Age:Diet
 - Null model: Glucose ~ (1 | MouseID) + (1 | DateCollect) + Diet + Age
...
The significance of the age-diet interaction effect conditioned on bodyweight was assessed by comparing the full and null models shown below:  
 - Full model: Glucose ~ (1 | MouseID) + (1 | DateCollect) + Bodyweight + Diet + Age + Age:Diet
 - Null model: Glucose ~ (1 | MouseID) + (1 | DateCollect) + Bodyweight + Diet + Age
 
If the results showed that the age effect (indpendent of weight or conditioned on weight) on glucose significantly (p < 0.05) varies by diet, the significance of the pairwise differences between each diet were assessed using a Tukey correction for multiple tests (R/emmeans).
...
There are four primary output files which contain the results of the analysis and information on the phenotype(s)  
 ...
 - intervention_age_effects_conditioned_on_diet.csv: same as above but only using data from th 5 and 16 month timepoint
    <br>

 - postintervention_age_effects_conditioned_on_diet.csv: same as above but only using data from th 16 and 28 month timepoint
    <br>  
...
<br>
 ...
 - intervention_diet_age_interaction_effects.csv: same as above but only using data from th 5 and 16 month timepoint
    <br>

 - postintervention_diet_age_interaction_effects.csv: same as above but only using data from th 16 and 28 month timepoint
    <br>  
