...
DATASET <- readRDS('data/Glucose_diet_age_dataset.rds')
...
DATASET$PostIntervention_Age_Effects <- AGE_EFFECTS
...
DATASET$PostIntervention_DietAge_Effects <- DIETAGE_EFFECTS
...
write_csv(
  ...
  paste0(
    ...
    '_postintervention_age_effects_conditioned_on_diet.csv'
  )
)
...
write_csv(
  ...
  paste0(
    'results/', DATASET$Assay,
    '_postintervention_diet_age_interaction_effects.csv'
  )
)
